...
dataset = SceneNNDataset(root='data/scenenn_seg_76/scenenn_seg/', training=False)
loader = data.DataLoader(dataset, batch_size=args.batch_size, shuffle=True, **kwargs)

cudnn.benchmark = True

num_classes = 41
model = PointDenseClassifier(num_points=4096, k=num_classes)
model.load_state_dict(torch.load('checkpoints/PointNet_scenenn_epoch070.pth'))
if args.cuda:
    model.cuda()
model.eval()

per_class_correct = np.zeros(41, dtype=np.long)
per_class_labels = np.zeros(41, dtype=np.long)
...
for i, (points, labels) in enumerate(loader):
    ...
    labels = labels.view(-1, 1)[:, 0].cpu()

    pred_choice = pred.data.max(1)[1].cpu()
    correct = pred_choice.eq(labels.data)
